...
const guint flash_interval_ms = 500;   /* 500 ms = 0.5s blink; change as desired */

// (unused scrolling ticker data retained but not used)
...
/* toggle ticker label visibility each interval */
static gboolean flash_ticker_cb(gpointer user_data) {
    if (!GTK_IS_WIDGET(ticker_label)) return G_SOURCE_REMOVE;

    gboolean vis = gtk_widget_get_visible(ticker_label);
    gtk_widget_set_visible(ticker_label, !vis);
    return G_SOURCE_CONTINUE;
}

// ===================== (old scrolling functions left unused) =====================
...
static gboolean set_paned_ratios(gpointer user_data) {
    ...
    if (tf_w > 1 && tf_h > 1) {
        gtk_widget_set_size_request(ticker_fixed, tf_w, tf_h);
    }

    int tl_w = gtk_widget_get_allocated_width(ticker_label);
    int tl_h = gtk_widget_get_allocated_height(ticker_label);
    if (tl_w > 1 && tl_h > 1) {
        gtk_widget_set_size_request(ticker_label, tl_w, tl_h);
    }

    gtk_widget_set_hexpand(ticker_fixed, FALSE);
    gtk_widget_set_vexpand(ticker_fixed, FALSE);
...
int main(int argc, char *argv[]) {
    ...
    /* --- Force the window to match the monitor size (modern Gdk API) --- */
    ...
    // removed scrolling ticker timer:
