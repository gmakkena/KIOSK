...
/* Render one token into a GdkPixbuf (in memory). Caller must g_object_unref(). */
                                     ...
                                     const char *num_font, const char *lab_font)
{
    ...
    double fr = 0.0, fgc = 0.0, fb = 0.0;
    if (num_fg_hex && sscanf(num_fg_hex, "#%02x%02x%02x", &r, &g, &b) == 3) {
        fr = r / 255.0; fgc = g / 255.0; fb = b / 255.0;
    } else {
        fr = 0.8; fgc = 0.0; fb = 0.0; // fallback red-ish
    }
    ...
    // label (small) with configurable color
...
// Refresh three images on UI thread
...
// ===================== Auto-resize handler =====================
static void on_image_size_allocate(GtkWidget *widget, GdkRectangle *alloc, gpointer user_data) {
    const char *which = (const char *)user_data;
    if (!which) return;

    if (strcmp(which, "current") == 0) {
        GdkPixbuf *pb = render_token_pixbuf(widget, current_token, "Current Draw",
                                           "#FFDAB9", "#FF0000", "#202020",
                                           0.7, 0.15, "Liberation Sans Bold", "Liberation Sans");
        if (pb) { gtk_image_set_from_pixbuf(GTK_IMAGE(widget), pb); g_object_unref(pb); }
    } else if (strcmp(which, "previous") == 0) {
        GdkPixbuf *pb = render_token_pixbuf(widget, previous_token, "Previous Draw",
                                           "#FFDAB9", "#0000FF", "#000080",
                                           0.6, 0.12, "Liberation Sans Bold", "Liberation Sans");
        if (pb) { gtk_image_set_from_pixbuf(GTK_IMAGE(widget), pb); g_object_unref(pb); }
    } else if (strcmp(which, "preceding") == 0) {
        GdkPixbuf *pb = render_token_pixbuf(widget, preceding_token, "Preceding Draw",
                                           "#FFDAB9", "#8B4513", "#5C4033",
                                           0.6, 0.12, "Liberation Sans Bold", "Liberation Sans");
        if (pb) { gtk_image_set_from_pixbuf(GTK_IMAGE(widget), pb); g_object_unref(pb); }
    }
...
static gboolean set_paned_ratios(gpointer user_data) {
    ...
    /* Fallbacks when allocations are zero on early runs */
    if (top_alloc.height == 0) top_alloc.height = 120;
    ...
    gtk_paned_set_position(GTK_PANED(top_pane), (gint)(top_alloc.height * 0.12));   // top label
    gtk_paned_set_position(GTK_PANED(outermost), (gint)(outermost_alloc.height * 0.88)); // content vs ticker
    gtk_paned_set_position(GTK_PANED(outer), (gint)(outer_alloc.width * 0.72));     // left/right
    gtk_paned_set_position(GTK_PANED(inner), (gint)(inner_alloc.height * 0.70));    // sub split

    // Force visibility of top label and ticker
    gtk_widget_show(top_label);
    gtk_widget_show(ticker_fixed);
    gtk_widget_show(ticker_label);

    // update tokens once layout is done
        ...
        if (n > 0) {
            for (int i = 0; i < n; ++i) {
                ...
                if (c == '\r' || c == '\n') {
                    ...
                    char *save = NULL;
                    char *field0 = strtok_r(p, " ", &save);
                    char *field1 = strtok_r(NULL, " ", &save);
                    char *field2 = strtok_r(NULL, " ", &save);
                    if (field0 && field0[0] == ':') {
                        if (field1 && field2 && strcmp(field1, "1") == 0) {
                            shift_tokens(field2);
                            g_idle_add(update_ui_from_serial, NULL);
                        } else if (field1 && strcmp(field1, "3") == 0) {
                            g_idle_add(show_fullscreen_gif, (gpointer)"rolling.gif");
                        }
                    } else if (field0 && strcmp(field0, "$M") == 0 && field1) {
...
int main(int argc, char *argv[]) {
    ...
    // Load CSS (style.css should contain rules that remove paned separators)
    ...
    // Connect resize handlers to auto-redraw token images on allocation changes
    g_signal_connect(current_image, "size-allocate", G_CALLBACK(on_image_size_allocate), "current");
    g_signal_connect(previous_image, "size-allocate", G_CALLBACK(on_image_size_allocate), "previous");
    g_signal_connect(preceding_image, "size-allocate", G_CALLBACK(on_image_size_allocate), "preceding");

    gtk_widget_show_all(window);

    // fullscreen & ensure allocation stabilises
    ...
    // Set paned ratios twice to ensure layout stabilises
    g_timeout_add(200, set_paned_ratios, NULL);
    g_timeout_add(800, set_paned_ratios, NULL);

    // Ensure gif overlay hidden to start
