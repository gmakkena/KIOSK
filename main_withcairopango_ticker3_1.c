...
static gboolean set_paned_ratios(gpointer user_data) {
    ...
    // Refresh token images if visible
...
static gboolean show_please_wait_cb(gpointer data)
{
    if (please_wait_label)
        gtk_widget_set_opacity(please_wait_label, 1.0);
    return G_SOURCE_REMOVE;
}
                        ...
                        f2)
                    {
                        ...
                        if (!first_token_received) {
                            ...
                            if (is_bulk_arrival) {
                                ...
                                // Hide token display and show "Please wait..."
                                if (outermost)
                                   gtk_widget_set_opacity(outermost, 0.0);
                                g_idle_add(show_please_wait_cb, NULL);
                            } else {
                                ...
                                // Show token display and widgets, hide "Please wait..."
                                if (outermost)
                                 gtk_widget_set_opacity(outermost, 1.0);

gtk_widget_set_opacity(current_image, 1.0);
gtk_widget_set_opacity(previous_image, 1.0);
gtk_widget_set_opacity(preceding_image, 1.0);
                                g_idle_add(hide_please_wait_cb, NULL);
...
int main(int argc, char *argv[]) {
    ...
    gtk_widget_set_opacity(current_image, 0.0);
gtk_widget_set_opacity(previous_image, 0.0);
gtk_widget_set_opacity(preceding_image, 0.0);
gtk_widget_set_opacity(outermost, 0.0);

// "Please wait..." should be visible
gtk_widget_set_opacity(please_wait_label, 1.0);
