...
void generate_token_image(GtkWidget *widget, const char *number, const char *label,
    ...
    int number_size = (int)(base * 0.58);
    int label_size  = (int)(base * 0.15);
...
// ---------- Finalize ticker setup after widget allocation ----------
...
gboolean set_paned_ratios(gpointer user_data) {
    ...
    gtk_paned_set_position(GTK_PANED(inner_pane), (int)(alloc.height * 0.65));

    g_timeout_add(50, update_token_images, NULL);

    gtk_label_set_text(GTK_LABEL(ticker_label), "Aurum Smart Tech");
    PangoAttrList *attrs = pango_attr_list_new();
    pango_attr_list_insert(attrs, pango_attr_size_new((int)(screen_height * 0.1 * PANGO_SCALE)));
