...
// ---------- Show Fullscreen GIF with Black Background ----------
gboolean show_fullscreen_gif(gpointer filename_ptr) {
    const char *filename = (const char *)filename_ptr;

    char command[512];
    snprintf(command, sizeof(command),
             "mpv --no-audio --fs --loop=inf --really-quiet \"%s\" & sleep 5; pkill -f 'mpv.*%s'",
             filename, filename);
    system(command);
    return FALSE;
}
...
gboolean fade_in_step(gpointer data) {
    FadeInData *fade = (FadeInData *)data;
    fade->opacity += 0.05;
    if (fade->opacity >= 1.0) {
        gtk_widget_set_opacity(fade->widget, 1.0);
        g_free(fade);
        return G_SOURCE_REMOVE;
    } else {
        gtk_widget_set_opacity(fade->widget, fade->opacity);
        return G_SOURCE_CONTINUE;
    }
}

void start_fade_in(GtkWidget *widget) {
    gtk_widget_set_opacity(widget, 0.0);
    FadeInData *fade = g_new(FadeInData, 1);
    fade->widget = widget;
    fade->opacity = 0.0;
    g_timeout_add(30, fade_in_step, fade);
}


// ---------- Only Updates GTK Images ----------
gboolean refresh_images_on_ui(gpointer user_data) {
    ...
    start_fade_in(current_image);
    //start_glow_effect(current_image);
start_fade_in(previous_image);
start_fade_in(preceding_image);
    return FALSE;
}
...
void *image_generator_thread(void *arg) {
    generate_token_image(current_image, current_token, "Current Draw", "current.png", "peachpuff", "red",
                         ...
                         "/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf", "/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf");
...
gboolean set_paned_ratios(gpointer user_data) {
    ...
    //gtk_widget_set_margin_top(top_label, (int)(h * 0.5));
    gtk_widget_set_margin_top(top_pane, (int)(h * 0.02));
